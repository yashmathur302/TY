...
function handleLike(btn) {
    ...
    .then(function(data) {
        ...
        if (data.liked) {
            btn.classList.remove('text-gray-500', 'dark:text-white/60');
        ...
        if (countEl) {
            countEl.textContent = data.count > 0 ? data.count : '0';
            if (data.count > 0) { countEl.classList.remove('hidden'); }
...
function handleShare(btn) {
    ...
    .catch(function(e) { console.error('Share failed', e); });
}
...
function showUsersList(postId, type, title) {
    ...
    .then(function(data) {
        if (!data.users || data.users.length === 0) {
            listEl.innerHTML = '<div class="text-center py-6 text-gray-400">No one yet.</div>';
            return;
        }
    ...
    .catch(function() {
        listEl.innerHTML = '<div class="text-center py-6 text-red-400">Failed to load.</div>';
    });
}
</script>
